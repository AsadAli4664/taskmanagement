    ...
    <div class="card card-info card-outline">
        ...
        <div class="card-body">
            <form action="{{route('task.update', $task->id)}}" method="POST">
                ...
                <input type="datetime-local" class="form-control" name="duedate" id="duedate" value="<?= isset($task) ? date('Y-m-d\TH:i', strtotime($task->duedate)) : '' ?>">
                <label class="form-label required mt-2">Description</label>
                    <textarea class="form-control" rows="3" name="description" placeholder="Description">{{ old('description') ?? $task->description }}</textarea>
